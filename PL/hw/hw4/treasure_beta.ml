...
let rec string_of_list string_of_el l =
    match l with
    | head :: tail -> (string_of_el head) ^ ", " ^ (string_of_list string_of_el tail)
    | [] -> ""
let rec string_of_key key =
    match key with
    | Bar -> "-"
    | Node (left, right) -> "(" ^ (string_of_key left) ^ ", " ^ (string_of_key right) ^ ")" 
let rec string_of_key_list = string_of_list string_of_key
let rec string_of_treasure treasure =
    match treasure with
    | StarBox -> "*"
    | NameBox (name) -> name
let rec string_of_implication_tree implT =
    match implT with 
    | Leaf (key, treasure) -> "L(" ^ (string_of_key key) ^ ", " ^ (string_of_treasure treasure) ^ ")"
    | SinglePath (key, name, implT) -> "S(" ^ (string_of_key key) ^ ", " ^ name ^ ", " ^ (string_of_implication_tree implT) ^ ")"
    | DoublePath (key, left, right) -> "D(" ^ (string_of_key key) ^ "," ^ (string_of_implication_tree left) ^ ", " ^ (string_of_implication_tree right) ^ ")"
let map = Guide("x", Guide("y", Guide("z", Branch(Branch(End(NameBox("z")), End(NameBox("y"))), End(NameBox("x")))))) 
(* let _ = print_endline (string_of_implication_tree (mapToImplicationTree map None)) *)
let testTree1 = End(NameBox("x"))
let testTree2 = Guide("x", End(NameBox("x")))
let testTree3 = Branch(Guide("x", End(NameBox("x"))), End(StarBox))
let testTree4 = Branch(Guide("x", Branch(End(NameBox("x")), End(NameBox("x")))), End(StarBox))
let testTree5 = Branch(Guide("x", End(NameBox("x"))), Branch(Guide("y", End(NameBox("y"))), End(StarBox)))
let _ = print_endline (string_of_implication_tree (mapToImplicationTree map ImplNone))
let _ = print_endline (string_of_key_list (getReady map))
