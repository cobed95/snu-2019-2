...
let rec trans_obj : Sm5.obj -> Sonata.obj = function
  ...
  | Sm5.Fn (arg, command) -> 
    Sonata.Fn (arg, Sonata.BIND "#continue" :: (trans' command) @ 
    [Sonata.PUSH (Sonata.Id "#continue"); Sonata.PUSH (Sonata.Val Sonata.Unit); Sonata.MALLOC; Sonata.CALL])
...
and trans' : Sm5.command -> Sonata.command = function
  ...
  | Sm5.CALL :: cmds -> 
    Sonata.BIND "#func_arg" :: Sonata.PUSH (Sonata.Id "#func_arg") :: Sonata.STORE ::
    Sonata.MALLOC :: Sonata.BIND "#func" :: Sonata.PUSH (Sonata.Id "#func") :: Sonata.STORE ::
    Sonata.PUSH (
      Sonata.Fn ("#x", 
        trans' cmds @ 
        [Sonata.PUSH (Sonata.Id "#continue"); 
         ...
         Sonata.MALLOC; 
         Sonata.CALL])) :: 
    Sonata.PUSH (Sonata.Id "#func") :: Sonata.LOAD :: Sonata.PUSH (Sonata.Id "#func_arg") :: Sonata.LOAD ::
    Sonata.PUSH (Sonata.Id "#func_arg") ::
    Sonata.CALL :: []
    (* failwith "TODO : fill in here" *)
...
let trans : Sm5.command -> Sonata.command = fun command ->
  Sonata.PUSH (Sonata.Fn("#x", [Sonata.UNBIND; Sonata.POP])) :: Sonata.BIND "#continue" :: 
  (trans' command) 
  (* failwith "TODO : fill in here" *)
