...
let rec eval env exp = 
  ...
  | App (e1, e2) ->
    ...
    (match c with 
    | Fun (x, e) -> 
      (* if x = "x_4" then (print_endline "arg to x_4: "; pp 0 e2; print_endline "")
      else if x = "x_6" then (print_endline "arg to x_6: "; pp 0 e2; print_endline "")
      else if x = "x_7" then (print_endline "arg to x_7: "; pp 0 e2; print_endline "")
      else (); *)
      eval (bind env' x v2) e
    | RecFun (f, x, e) ->  
  ...
  | Add (e1, e2) -> 
    ...
    let _ = print_endline "v1" in
    let _ = pp 0 e1; print_endline "" in
    let _ = print_endline (string_of_int (getNum v1)) in
    let _ = print_endline "v2" in
    let _ = pp 0 e2; print_endline "" in
    let _ = print_endline (string_of_int (getNum v2)) in *)
