...
struct
  ...
  let rec eval env mem exp = 
    ...
    | LET (decl, parent) -> (* TODO: implement this *)
      ...
      | REC (fun_id, arg_id, body) -> (* consider mem change in the spec doc *)
        ...
        | Fun (x, e) -> 
          ...
          eval (env @+ (fun_id, Closure (RecFun (fun_id, arg_id, body), env'))) m' parent
        | _ -> raise (TypeError "Repetitive recursive function definition"))) *)
      (* failwith "Unimplemented" *)
    | IF (e1, e2, e3) ->
      let (v1, m') = eval env mem e1 in
