  ...
  *)
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
       ...
       * let _ = ... 
       *) 
      let rec add_no_dups a a_list =
        if List.mem a a_list then a_list
        else a :: a_list in
      let concat_no_dups a_list b_list =
        let no_dups = List.filter (fun x -> not (List.mem x a_list)) b_list in
        a_list @ no_dups in
      let extract_env_cont c = match c with (_, e) -> e in
      let envs = e :: (List.map extract_env_cont k) in
      let rec add_loc env result = 
        match env with
        | head :: tail ->
          (match head with
          | (_, Loc l) -> 
            let value = 
              try List.assoc l m
              with Not_found -> Unit in
            (match value with
            | R r ->
              let rec_locs = List.map (fun (_, loc) -> loc) r in
              let result' = add_no_dups l result in
              let result'' = concat_no_dups result' rec_locs in
              add_loc tail result''
            | _ ->
              let new_result = add_no_dups l result in
              add_loc tail new_result)
          | (_, Proc (_, _, e)) -> 
            let new_result = add_loc e result in
            add_loc tail new_result)
        | [] -> result in
      let rec add_loc_all env_list result = 
        match env_list with
        | head :: tail -> 
          let new_result = add_loc head result in
          add_loc_all tail new_result
        | [] -> result in
      let reachable_locs_naive = add_loc_all envs [] in
      let reachable_bases = List.map (fun (base, _) -> base) reachable_locs_naive in
      let locs_reachable_from_base = 
        List.filter (fun (base, _) -> List.mem base reachable_bases) (List.map (fun (l, _) -> l) m) in
      let _ = reachable_locs := concat_no_dups reachable_locs_naive locs_reachable_from_base in
